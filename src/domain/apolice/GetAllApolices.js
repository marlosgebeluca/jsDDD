...
class GetAllApolices extends Operation {
  ...
  async execute() {
    ...
    try {
      const apolices = await this.apoliceRepository.getAll({
      ...
      for (var apolice of apolices) {
        
        const endossos = await this.apoliceRepository.findEndossos(apolice.id);
...
GetAllApolices.setOutputs(['SUCCESS', 'ERROR']);

module.exports = GetAllApolices;
